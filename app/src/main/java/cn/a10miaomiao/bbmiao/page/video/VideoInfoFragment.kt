...
import cn.a10miaomiao.bbmiao.comm.recycler._miaoMultiAdapter
import cn.a10miaomiao.bbmiao.comm.recycler.footerViews
import cn.a10miaomiao.bbmiao.comm.recycler.headerViews
import cn.a10miaomiao.bbmiao.comm.recycler.lParams
...
//            },
        ...
        when (menuItem.key) {
            ...
            2 -> {
                ...
                var shareIntent = Intent().apply {
                    ...
                    putExtra(
                        Intent.EXTRA_TEXT,
                        "${info.title} https://www.bilibili.com/video/${viewModel.bvId}"
                    )
                }
            ...
            3 -> {
                ...
                if (info != null) {
                    ...
                    val args = VideoAddFavoriteFragment.createArguments(info.aid.toString())
    ...
    private fun playVideo(page: bilibili.app.archive.v1.Page) {
        val info = viewModel.arcInfo
        if (info != null) {
    ...
    private val handleUpperClick = View.OnClickListener {
        viewModel.arcInfo?.let { info ->
            toUser(it, info.author?.mid.toString())
        }
    }
...
//        CoverActivity.launch(requireContext(), viewModel.id)
            ...
            val args = VideoPagesFragment.createArguments(
                video = VideoPagesParam(
                    ...
                    pages = viewModel.pages.map {
                        VideoPagesParam.Page(it.cid.toString(), it.part)
                    }
                )
            )
    ...
    private val handleVideoHistoryClick = View.OnClickListener {
        val history = viewModel?.historyInfo ?: return@OnClickListener
    ...
    private val handleTagsItemClick = OnItemClickListener { adapter, view, position ->
        ...
        val args = SearchResultFragment.createArguments(item.name)
    ...
    private val handleRelateItemClick = OnItemClickListener { adapter, view, position ->
        ...
        if (item is bilibili.app.view.v1.Relate) {
            if (item.goto == "av") {
                val args = createArguments(item.aid.toString())
                Navigation.findNavController(view)
                    .navigate(actionId, args)
            } else {
                val url = item.uri
...
//                PopTip.show("你点击了@用户 内容是：$content")
    ...
    val pageItemUi = miaoBindingItemUi<bilibili.app.archive.v1.Page> { item, index ->
        frameLayout {
            ...
            val enabled = playerStore.state.cid != item.cid.toString()
...
//                    _show = currentPosition >= 0
    ...
    val upperItemUi = miaoBindingItemUi<bilibili.app.view.v1.Staff> { item, index ->
        verticalLayout {
    ...
    fun MiaoUI.upperView(): View {
        return verticalLayout {
            ...
            val isMutableUpper = viewModel.staffs.isNotEmpty()
            ...
            views {
                ...
                +horizontalLayout {
                    ...
                    views {
                        +rcImageView {
                            isCircle = true
                            _network(viewModel.arcInfo?.author?.face, "@200w_200h")
                        }..lParams {
                            height = dip(40)
                            width = dip(40)
                        }
                        ...
                        +verticalLayout {
                            ...
                            views {
                                +textView {
                                    setTextColor(config.foregroundColor)
                                    _text = viewModel.arcInfo?.author?.name ?: ""
                                }
                                +textView {
                                    textSize = 12f
                                    setTextColor(config.foregroundAlpha45Color)
                                    _text = "发表于 " + NumberUtil.converCTime(viewModel.arcInfo?.pubdate)
                                }
                            }
    ...
    val itemTagUi = miaoBindingItemUi<bilibili.app.view.v1.Tag> { item, index ->
        frameLayout {
            views {
                +frameLayout {
    ...
    fun MiaoUI.headerView(): View {
        val videoInfo = viewModel.arcInfo
        return verticalLayout {
            views {
                +horizontalLayout {
                    views {
                                ...
                                +horizontalLayout {
                                    ...
                                    views {
                                        ...
                                        +textView {
                                            textSize = 12f
                                            setTextColor(config.foregroundAlpha45Color)
                                            _text = NumberUtil.converStringOrNull(videoInfo?.stat?.view) ?: ""
                                        }..lParams {
                                            leftMargin = dip(3)
                                            rightMargin = dip(16)
                                        }
                                        ...
                                        +textView {
                                            textSize = 12f
                                            setTextColor(config.foregroundAlpha45Color)
                                            _text = NumberUtil.converStringOrNull(
                ...
                +horizontalLayout {
                    ...
                    val history = viewModel.historyInfo
                    _show = history != null && history.progress > 0
                            && playerStore.state.aid != videoInfo?.aid?.toString()
                ...
                +expandableTextView {
                    ...
                    _setContent(viewModel.arcInfo?.desc ?: "")
    ...
    val relateItemUi = miaoBindingItemUi<bilibili.app.view.v1.Relate> { item, index ->
        videoItem(
            title = item.title,
            pic = item.pic,
            upperName = item.author?.name,
            playNum = NumberUtil.converStringOrNull(item.stat?.view),
            damukuNum = NumberUtil.converStringOrNull(item.stat?.danmaku),
